...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 22 13:58:22 2021
...
if IS_SSTR81 : # SSH_R81 + SST_R81 to SSH_R27
    ...
    for i in np.arange(7) : #ou (9) ?
        ArchiA = Conv2D(36,(6,6), activation=factiv, padding='same',kernel_initializer=init)(ArchiA)    
        #ArchiA = Conv2D(6,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiA)
...
for i in np.arange(3) :
    ...
    #ArchiB = Conv2D(3,(3, 3), activation=factiv, padding='same',kernel_initializer=init)(ArchiB)
...
for i in np.arange(3):
    ...
    #ArchiC= Conv2D(6,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiC)
...
# 4eme etage SSH_R03+ SST_R01 to SSH_R01
ArchiD = UpSampling2D((upfactor,upfactor))(ArchiC)
if IS_SSTR01 and 1:
    ArchiD= concatenate([ArchiD,ISSTR01], axis=3)#CHANEL LAST
#for i in np.arange(1):
#    ArchiD= Conv2D(12,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiD)
#ArchiD=Conv2D(12,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiD)
for i in np.arange(2):
    ArchiD=Conv2D(13,(3,3), activation=factiv, padding='same',kernel_initializer=init)(ArchiD)
#((taille de l'image - taille kernel +2*padding)/ (stride)) + 1
ArchiD= Conv2D(1,(1,1), activation=factout, padding='same',kernel_initializer=init,name='archi03to01')(ArchiD)
ArchiOut.append(ArchiD)


# 5eme �tage : H_R03+SST_R03 to U_R03 and V_R03
...
#     ArchiUV  = concatenate([ArchiD, ISSTR01], axis=3);# CHANEL LAST
#     ArchiUV  = Conv2D(nc,(ml,mc), activation=factiv, padding='same',kernel_initializer=init)(ArchiUV) ##
# else :
#     ArchiUV = Conv2D(nc,(ml,mc), activation=factiv, padding='same',kernel_initializer=init)(ArchiD) ##
...
#
    ...
    print("L'entrainement est allé jusqu'au bout des "+str(Niter)+" epoch" )
    #
    print("#Visualisation de la courbes d'erreur en apprentissage et de performances")
    plt.figure()
    LH = []
...
def setresult(Mdl, x_set, strset, VXout_brute, calX, flag_rms=0, flag_scat=0,flag_scat_border=0,
        ...
        if not RESBYPASS :
            if LIMLAT_NORDSUD <= 0 :
                result("brute scalled", strset, varOut[i], VXout_brute[i], y_scale[i],
                       flag_rms=flag_rms, flag_scat=flag_scat,flag_scat_border=flag_scat_border, flag_histo=flag_histo,
...
if TEST_ON and FLGT_RES : # sur TEST
    setresult(Mdl, x_test, "TEST", VTout_brute, calT, flag_rms=FLGT_RMS,
              flag_scat=FLGT_SCAT,flag_scat_border=FLGT_SCAT_BORDER, flag_histo=FLGT_HISTO, nbins=NBINS,
