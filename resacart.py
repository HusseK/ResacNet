...
varlue      = [i.decode() for i in varlue]
...
if IS_SSTR81 : # SSH_R81 + SST_R81 to SSH_R27
    ...
    for i in np.arange(7) : #ou (9) ?
        ArchiA = Conv2D(36,(6,6), activation=factiv, padding='same',kernel_initializer=init)(ArchiA);
    ArchiA = Conv2D(1,(2,2), activation=factout, padding='same',kernel_initializer=init,name='archiA')(ArchiA); 
...
ArchiB = Conv2D(1,(1, 1), activation=factout, padding='same',kernel_initializer=init,name='archiH')(ArchiB)
ArchiOut.append(ArchiB);
#
# 3�me �tage : H_R09+SST_R09 to U_R09 and V_R09
nc, ml, mc, nl = (24, 5, 5, 2);
if IS_SSTR09 : # SST R09 utilis� pour trouver SSU R09 et SSV R09
    #ArchiUV  = concatenate([ArchiB, ISSTR09], axis=1);# CHANEL FIRST
    ArchiUV  = concatenate([ArchiB, ISSTR09], axis=3);# CHANEL LAST
...
#======================================================================
if RUN_MODE=="RESUME" :
    print("Reload des poids d'un model pr�alablement sauvegard�");
    Mdl.load_weights(Mdl2reload+"valid.ckpt"); 
    print("reload wei, done that");
#        
    ...
    if VALID_ON == 0 : # Pas d'usage de l'ensemble de validation (run 'simple)
        ...
        H = Mdl.fit(x_train, y_train, verbose=2, epochs=Niter, batch_size=Bsize,
                shuffle=True, callbacks=[checkpoint], validation_data=(x_valid, y_valid));          
...
if VALID_ON == 4 :
    if RUN_MODE == "LEARN" :
        ...
        Mdl.load_weights(Mdl2save+"valid.ckpt"); 
...
def plot_history(lhist):
  ...
  for i in range(nb_loss):
    plt.plot(x,lhist.history[list(lhist.history.keys())[i]],c=color[i],label=list(lhist.history.keys())[i])

  plt.legend()
  plt.savefig("SaveModel/losses_apprentissage.png",dpi=600)
  plt.show()
plot_history(H)
